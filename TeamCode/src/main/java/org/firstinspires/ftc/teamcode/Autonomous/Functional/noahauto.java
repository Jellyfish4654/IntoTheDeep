...
@Config
@Autonomous(name = "noahauto", group = "Autonomous")
public class noahauto extends BaseOpMode {
...
//                .strafeToConstantHeading(new Vector2d(-44, 10))
                .strafeToConstantHeading(new Vector2d(-15*scale, 40*scale))

                .splineToLinearHeading(new Pose2d(-30*scale, 5*scale, Math.toRadians(85)), Math.toRadians(0))
                .lineToX(-42.5*scale)
                .strafeToConstantHeading(new Vector2d(-42.5*scale, 52.5*scale));

        Pose2d pushSamplePose = new Pose2d(-43*scale, 52.5*scale, Math.toRadians(85));
...
//                .strafeToConstantHeading(new Vector2d(-54, 57));
        ...
        Pose2d pushSamplePoseTwo = new Pose2d(-54*scale, 57*scale, Math.toRadians(85));

        TrajectoryActionBuilder readyGrabSpecimen = drive.actionBuilder(pushSamplePose)
                .strafeToConstantHeading(new Vector2d(-40*scale, 53*scale))
                .turn(Math.toRadians(180));

        Pose2d readyGrabSpecimenPose = new Pose2d(-42*scale, 53*scale, Math.toRadians(268));

        TrajectoryActionBuilder grabSpecimen = drive.actionBuilder(readyGrabSpecimenPose)
                .strafeToConstantHeading(new Vector2d(-45*scale, 56.5*scale));

        Pose2d grabSpecimenPose = new Pose2d(-45*scale, 56.5*scale, Math.toRadians(268));

        TrajectoryActionBuilder hangSecondSpecimen = drive.actionBuilder(grabSpecimenPose)
                .strafeToLinearHeading(new Vector2d(5*scale, 33*scale), Math.toRadians(90));

        Pose2d hangedSecondSpecimenPose = new Pose2d(5*scale, 33*scale, Math.toRadians(90));

        TrajectoryActionBuilder grabThirdSpecimen = drive.actionBuilder(hangedSecondSpecimenPose)
                .strafeToConstantHeading(new Vector2d(-43*scale, 53*scale))
                .turn(Math.toRadians(170))
                .strafeToConstantHeading(new Vector2d(-43*scale, 58*scale));

        Pose2d grabThirdSpecimenPose = new Pose2d(-43*scale, 58*scale, Math.toRadians(263.5));

        TrajectoryActionBuilder hangThirdSpecimen = drive.actionBuilder(grabThirdSpecimenPose)
                .strafeToLinearHeading(new Vector2d(-2*scale, 31*scale), Math.toRadians(84.25));
